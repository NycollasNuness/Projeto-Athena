...
const EditarFlashcards = () => {
  const navigate = useNavigate()
  const { id } = useParams()
  const { updateDeck, getDeckById } = useDecks()
  ...
  useEffect(() => {
    if (id) {
      ...
      if (deck) {
        setSubjectName(deck.name)
        setFlashcards(deck.flashcards.length > 0 ? deck.flashcards : [{ id: '1', front: '', back: '' }])
      } else {
        toast({
          title: "Erro",
  ...
  const handleSave = () => {
    ...
    if (id) {
      updateDeck(id, subjectName, validFlashcards)
      
      toast({
        title: "Sucesso!",
        description: `Matéria "${subjectName}" atualizada com ${validFlashcards.length} flashcard${validFlashcards.length > 1 ? 's' : ''}.`
      })
    ...
    <div className="p-8 bg-background min-h-screen">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div className="inline-block bg-study-peach px-6 py-3 rounded-2xl">
...
export default EditarFlashcards